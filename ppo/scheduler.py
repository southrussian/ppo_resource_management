...
class SurgeryQuotaScheduler(ParallelEnv):
    ...
    def __init__(self, render_mode=None, max_agents=12, max_days=7, max_episode_length=3):
    ...
    def reset(self, seed=None, options=None):
        ...
        # self.expert_logic = options['expert_logic'] if options is not None else False
        self.target_state = {day: (boundaries['min'] + boundaries['max']) / 2
                             for day, boundaries in options['target_state'].items()} if options is not None else {
            day: self.max_agents / self.max_days
        ...
        self.agents_data = options['agents_data'] if options is not None else {agent: {'active': True,
                                                                                       ...
                                                                                       'mutation_rate': 0.0}
                                                                               for (
                                                                               urgency, complexity, completeness), agent
                                                                               in zip(itertools.product(range(1, 4),
                                                                                                        range(0, 2),
                                                                                                        range(0, 2)),
                                                                                      self.agents)}
        for agent in self.agents:
            self.agents_data[agent]['position'] = self.rng.integers(0, 7, 1).item()
            self.agents_data[agent]['scaling_factor'] = ((self.agents_data[agent]['complexity']
    ...
    def step(self, actions):
        ...
        if any(terminations.values()) or any(truncations.values()):
            rewards = {agent: r - abs(self.observed_state[self.agents_data[agent]['position']] - self.target_state[
    ...
    def expert_defined_logic(self, agent):
        return 0.7 * (self.agents_data[agent]['scaling_factor'] / (self.agents_data[agent]['position'] + 1))
    ...
    def reward_map(self, agent):
        ...
        reward = -weighted_discrepancy_sum * self.agents_data[agent]['base_reward']
        if self.expert_logic:
            reward += self.expert_defined_logic(agent)
        return reward
